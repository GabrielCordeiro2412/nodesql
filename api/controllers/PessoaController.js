...
class PessoaController {
    ...
    static async pegaUmaMatricula(req, res){
        ...
        try {
            const umaMatricula = await database.Matriculas.findOne({where: {id: Number(matriculaId), estudante_id: Number(estudanteId)}, include: [
    ...
    static async pegaTodasAsMatriculasPorTurma(req, res){
        ...
        try {
            const todasAsMatriculas = await database.Matriculas.findAll({where: {turma_id: Number(turmaId)}})
            return res.status(200).json(todasAsMatriculas)
        } catch (error) {
            return res.status(500).json(error.message)
        }
